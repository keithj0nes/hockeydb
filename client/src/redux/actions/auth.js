...
export const createAccount = (userData) => async () => {
    const data = await request({ url: '/api/auth/signup', method: 'POST', session: userData, publicRoute: true });
    if (!data) return false;
    return true;
};
