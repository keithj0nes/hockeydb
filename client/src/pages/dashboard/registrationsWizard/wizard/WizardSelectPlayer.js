...
const WizardSelectPlayer = () => {
    ...
    const { fields, handleChange, errors, validate, setFieldsDangerously } =
        formData;
    ...
    const handleSubmit = async (e) => {
        ...
        setFieldsDangerously(
            {
                'First Name':
                    registeringPlayer.first_name ||
                    registeringPlayer.value?.['First Name'],
                'Last Name':
                    registeringPlayer.last_name ||
                    registeringPlayer.value?.['Last Name'],
                'Phone Number':
                    registeringPlayer.phone_number ||
                    registeringPlayer.value?.['Phone Number'],
    ...
    const associatedAccountsWhoCanRegister = associatedAccounts.filter(
        (aa) =>
            !submissions.some((s) => {
    ...
    return (
        <form onSubmit={handleSubmit} id="modal-wrapper">
            <div className="p-4 pb-0 max-w-lg mx-auto">
                ...
                <p className="text-sm">
                    We noticed previous registrations tied to your account. You
                    can register an existing player, or create a new one
                </p>
            </div>

            {errors['register-as'] && (
                <p className="pt-6 text-center text-red-500 text-sm pl-1">
                    {errors['register-as']}
                </p>
            )}
            ...
            {!!alreadyRegistered.length && (
                <div className="py-4 text-center">
                    <button
                        onClick={showModal}
                        className="p-3 text-blue-500 underline"
                        type="button"
                    >
                        View already registered players (
                        {alreadyRegistered.length})
                    </button>
                </div>
            )}
                ...
                <ul className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 p-5 m-auto">
                    {canRegister.map((item) => {
                        ...
                        const names = {
                            ...
                            initials: `${item.value['First Name'].charAt(
                                0
                            )}${item.value['Last Name'].charAt(0)}`,
                        };
                    ...
                    {associatedAccountsWhoCanRegister.map((item) => {
                        ...
                        const names = {
                            ...
                            initials: `${item.first_name.charAt(
                                0
                        ...
                        return (
                            ...
                            >
                                <input
                                    ...
                                    disabled={false}
                                    type="radio"
                                    value={isUser ? item.player_id : item.id}
                                    name="register-as"
                                    id={`register-${item.id}`}
                                    onChange={handleChange}
                                    checked={
                    ...
                    <li className="flex justify-center relative">
                        <input
                            ...
                            id="register-new"
                            onChange={handleChange}
                            checked={fields['register-as'] === String(0)}
                        />
                        <label
                        ...
                        >
                            <div className="bg-db-secondary rounded-full h-16 w-16 flex justify-center items-center m-auto">
                                ...
                                <svg
                                    className="w-8 h-8 text-white peer-checked:bg-blue-400"
                                    fill="none"
                                    stroke="currentColor"
                                    viewBox="0 0 24 24"
                            ...
                            <div className="py-4 text-center">New player</div>
                            <div className="py-0 text-center text-xl font-bold">
                                Start
                            </div>
                        </label>
            ...
            <div className="relative py-4 hidden">
                ...
                <div className="relative flex justify-center">
                    <span className=" bg-light-gray px-4 text-sm text-gray-500">
                        OR
                    </span>
                </div>
            </div>
            ...
            <div className="flex justify-center pt-6 relative hidden">
                ...
                <input
                    ...
                    id="register-new1"
                    onChange={handleChange}
                    checked={fields['register-as2'] === String(0)}
                />
                <label
                ...
                >
                    ...
                    <div className="py-4 text-center">New player</div>
                    <div className="py-0 text-center">Start Registration</div>
                </label>
            </div>
                ...
                <ul className="w-full max-w-xl m-auto text-sm font-medium text-gray-900 bg-white rounded-lg">
                    {alreadyRegistered.map((item) => {
                        ...
                        return (
                            <li
                                key={item.id}
                                className="flex justify-between py-2 px-4 w-full rounded-t-lg border-b border-gray-200 last:border-b-0"
                            >
                                <div>
                                    <h4>
