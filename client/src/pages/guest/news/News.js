...
const News = () => {
    ...
    return (
        <div className="">
            {news.map((newsItem) => (
                <div key={newsItem.id} className="bg-white p-2 sm:p-4 mb-6 rounded">
                    <h1 className="text-2xl sm:text-3xl">{newsItem.title}</h1>
                    ...
                    <div className="sm:flex">
                        ...
                        <h6 className="uppercase text-gray-700 text-xs pt-2 sm:pt-4">{format(parseISO(newsItem.created_at), 'P | p')}</h6>

                        {updated_at && (
                            <h6 className="uppercase text-gray-700 text-xs pt-2 sm:pt-4 pl-4">Updated: {format(parseISO(newsItem.updated_at), 'P')}</h6>
                        )}
                    ...
                    <div className="border-t border-gray-200 mt-3 pt-3 sm:mt-5 sm:pt-5">
                        <div className="--news-body" dangerouslySetInnerHTML={{ __html: newsItem.body }} />
                    </div>
                </div>
            ))}
        </div>
