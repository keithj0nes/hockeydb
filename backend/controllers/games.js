...
const getGames = async (req, res, next) => {
    try {
        ...
        return res.send({ status: 200, data: { games, games_count: games_count[0].count, seasons, divisions, teams, fromLoadMore }, message: 'Retrieved list of games' });
    } catch (error) {
...
const createGame = async (req, res, next) => {
    try {
        ...
        const game = await db.games.insert({ home_team: home_team.home_team, away_team: away_team.away_team, location_id, start_date, has_been_played: false });
        await db.game_season_division.insert({ game_id: game.id, season_id, division_id: home_team.division_id });
